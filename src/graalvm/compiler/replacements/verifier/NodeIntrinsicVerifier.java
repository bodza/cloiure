...
import graalvm.compiler.graph.Node.InjectedNodeParameter;
import graalvm.compiler.graph.Node.NodeIntrinsic;
import graalvm.compiler.nodeinfo.InputType;
import graalvm.compiler.nodeinfo.NodeInfo;
import graalvm.compiler.nodeinfo.StructuralInput.MarkerType;
...
public final class NodeIntrinsicVerifier extends AbstractVerifier
{
    ...
    {
        return env.getElementUtils().getTypeElement("graalvm.compiler.graph.Node").asType();
    }

    private TypeMirror stampType()
    {
        return env.getElementUtils().getTypeElement("graalvm.compiler.core.common.type.Stamp").asType();
    }

    private TypeMirror valueNodeType()
    {
        return env.getElementUtils().getTypeElement("graalvm.compiler.nodes.ValueNode").asType();
    }
    ...
    {
        return env.getElementUtils().getTypeElement("graalvm.compiler.nodeinfo.StructuralInput").asType();
    }

    private TypeMirror graphBuilderContextType()
    {
        return env.getElementUtils().getTypeElement("graalvm.compiler.nodes.graphbuilderconf.GraphBuilderContext").asType();
    }
