...
import static graalvm.compiler.core.common.GraalOptions.PrintProfilingInformation;
...
import static graalvm.compiler.debug.GraalError.guarantee;
...
import static graalvm.compiler.java.BytecodeParserOptions.TraceBytecodeParserLevel;
import static graalvm.compiler.java.BytecodeParserOptions.TraceInlineDuringParsing;
import static graalvm.compiler.java.BytecodeParserOptions.TraceParserPlugins;
...
import graalvm.compiler.nodes.calc.NormalizeCompareNode;
...
import graalvm.compiler.nodes.extended.StateSplitProxyNode;
import graalvm.compiler.nodes.extended.ValueAnchorNode;
import graalvm.compiler.nodes.graphbuilderconf.ClassInitializationPlugin;
import graalvm.compiler.nodes.graphbuilderconf.GraphBuilderConfiguration;
import graalvm.compiler.nodes.graphbuilderconf.GraphBuilderConfiguration.BytecodeExceptionMode;
...
import graalvm.compiler.nodes.java.FinalFieldBarrierNode;
...
import graalvm.compiler.nodes.java.RegisterFinalizerNode;
...
public class BytecodeParser implements GraphBuilderContext
{
            ...
            if (pluginResult == true)
            {
                ...
                for (Node n : newNodes)
                {
                    if (n instanceof StateSplit)
                    {
                        ...
                        assert stateSplit.stateAfter() != null || !stateSplit.hasSideEffect() : error("%s node added by plugin for %s need to have a non-null frame state: %s", StateSplit.class.getSimpleName(), targetMethod.format("%H.%n(%p)"), stateSplit);
                    }
                }
                try
                {
    ...
    protected void traceWithContext(String format, Object... args)
    {
        ...
        String s = format("%s%s (%s:%d) %s", nSpaces(getDepth()), method.isConstructor() ? method.format("%h.%n") : method.getName(), where.getFileName(), where.getLineNumber(), format(format, args));
        TTY.println(s);
    }
