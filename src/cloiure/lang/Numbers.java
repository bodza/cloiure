...
public class Numbers
{
 ...
 // }
    ...
    final static class BigDecimalOps extends OpsP
    {
        ...
        {
            MathContext mc = (MathContext) MATH_CONTEXT.deref();
            return (mc == null) ? toBigDecimal(x).multiply(toBigDecimal(y)) : toBigDecimal(x).multiply(toBigDecimal(y), mc);
        }
        ...
        {
            MathContext mc = (MathContext) MATH_CONTEXT.deref();
            return (mc == null) ? toBigDecimal(x).divideToIntegralValue(toBigDecimal(y)) : toBigDecimal(x).divideToIntegralValue(toBigDecimal(y), mc);
        }

        public Number remainder(Number x, Number y)
        {
            MathContext mc = (MathContext) MATH_CONTEXT.deref();
            return (mc == null) ? toBigDecimal(x).remainder(toBigDecimal(y)) : toBigDecimal(x).remainder(toBigDecimal(y), mc);
        }
