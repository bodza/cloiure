...
public class Compiler implements Opcodes
{
 ...
 // static final Symbol SYNTAX_QUOTE = Symbol.intern("cloiure.core", "syntax-quote");
 ...
 // static final Symbol IFN = Symbol.intern("cloiure.lang", "IFn");
            ...
            public Expr parse(C context, Object form)
            {
                ...
                if (!v.ns.equals(currentNS()))
                {
                    if (sym.ns == null)
                    {
                     ...
                     // throw Util.runtimeException("Name conflict, can't def " + sym + " because namespace: " + currentNS().name + " refers to:" + v);
                    else
 ...
 //             if (form.count() != 3)
 //                 throw new IllegalArgumentException("Wrong number of arguments, expecting: (try-finally try-expr finally-expr) ");
 //
 //             if (context == C.EVAL || context == C.EXPRESSION)
 //                 return analyze(context, RT.list(RT.list(FN, PersistentVector.EMPTY, form)));
 //
 //             return new TryFinallyExpr(analyze(context, RT.second(form)), analyze(C.STATEMENT, RT.third(form)));
 //         }
 //     }
 // }
                    ...
                    if (this.protocolOn != null)
                    {
                        ...
                        if (mmapVal == null)
                        {
                            throw new IllegalArgumentException("No method of interface: " + protocolOn.getName() +
                                    " found for function: " + fvar.sym + " of protocol: " + pvar.sym + " (The protocol method may have been defined before and removed.)");
                        }
                        String mname = munge(mmapVal.sym.toString());
                        List methods = Reflector.getMethods(protocolOn, args.count() - 1, mname, false);
                        if (methods.size() != 1)
                            throw new IllegalArgumentException("No single method: " + mname + " of interface: " + protocolOn.getName() +
     ...
     // String superName = null;
        ...
        static Expr parse(C context, ISeq form, String name)
        {
         ...
         // fn.thisName = name;
            ...
            try
            {
                fn.compile(fn.isVariadic() ? "cloiure/lang/RestFn" : "cloiure/lang/AFunction", (prims.size() == 0) ? null : prims.toArray(new String[prims.size()]), fn.onceOnly);
            }
            catch (IOException e)
            {
                throw Util.sneakyThrow(e);
        ...
        public void emitForDefn(ObjExpr objx, GeneratorAdapter gen)
        {
         ...
         //     gen.invokeConstructor(thunkType, Method.getMethod("void <init>(cloiure.lang.Var,String)"));
         // }
         // else
    ...
    static public class ObjExpr implements Expr
    {
     ...
     // }
        ...
        void compile(String superName, String[] interfaceNames, boolean oneTimeUse) throws IOException
        {
         ...
         //                                                    superName != null ? superName : (isVariadic() ? "cloiure/lang/RestFn" : "cloiure/lang/AFunction"), null);
            String source = (String) SOURCE.deref();
            ...
            if (source != null && SOURCE_PATH.deref() != null)
            {
             // cv.visitSource(source, null);
                String smap = "SMAP\n"
                    + ((source.lastIndexOf('.') > 0) ? source.substring(0, source.lastIndexOf('.')) : source) // : simpleName)
                    + ".java\n"
                    + "Cloiure\n"
                    + "*S Cloiure\n"
                    + "*F\n"
         ...
         // {
         //     cv.visitField(ACC_PRIVATE + ACC_STATIC + ACC_FINAL, varCallsiteName(i), IFN_TYPE.getDescriptor(), null, null);
         // }
        ...
        void emitValue(Object value, GeneratorAdapter gen)
        {
         ...
         //     gen.invokeStatic(Type.getType(KeywordCallSite.class), Method.getMethod("cloiure.lang.KeywordCallSite create(cloiure.lang.Symbol)"));
         // }
                     ...
                     // if (true) // RT.booleanCast(RT.WARN_ON_REFLECTION.deref()))
                        throw new IllegalArgumentException // RT.errPrintWriter().println
                            ( // source + ":" + line +
                             ...
                             (arg.hasJavaClass() ? arg.getJavaClass().getName() : "Object") + ", needed: " + primc.getName());
