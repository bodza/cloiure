...
{
    // @field
    private ResolvedJavaMethod ___cachedSubstitute;

    ///
    // The class in which the substitute method is declared.
    ///
    // @field
    private final Class<?> ___declaringClass;
    ...
    ///
    // @field
    private final Type[] ___parameters;

    // @field
    private final boolean ___originalIsStatic;

    // @field
    private final BytecodeProvider ___bytecodeProvider;
    ...
    public MethodSubstitutionPlugin(BytecodeProvider __bytecodeProvider, Class<?> __declaringClass, String __name, Type... __parameters)
    {
        ...
        this.___originalIsStatic = __parameters.length == 0 || __parameters[0] != InvocationPlugin.Receiver.class;
    }
    ...
    {
        if (this.___cachedSubstitute == null)
        {
            this.___cachedSubstitute = __metaAccess.lookupJavaMethod(getJavaSubstitute());
        }
        return this.___cachedSubstitute;
    }
    ...
    {
        if (Modifier.isStatic(__m.getModifiers()) && __m.getName().equals(this.___name))
        {
            if (this.___parameters.length == __m.getParameterCount())
            {
                Class<?>[] __mparams = __m.getParameterTypes();
                int __start = 0;
                if (!this.___originalIsStatic)
                {
                    __start = 1;
                    if (!__mparams[0].isAssignableFrom(InvocationPlugins.resolveType(this.___parameters[0], false)))
                    {
                        return false;
                    }
                }
                ...
                {
                    if (__mparams[__i] != InvocationPlugins.resolveType(this.___parameters[__i], false))
                    {
                        return false;
                    }
                }
    ...
    {
        for (Method __m : this.___declaringClass.getDeclaredMethods())
        {
    ...
    public boolean execute(GraphBuilderContext __b, ResolvedJavaMethod __targetMethod, InvocationPlugin.Receiver __receiver, ValueNode[] __argsIncludingReceiver)
    {
        ...
        return __b.intrinsify(this.___bytecodeProvider, __targetMethod, __subst, __receiver, __argsIncludingReceiver);
    }
}
